...
function init() {
    ...
    TweenMax.set('.cloud1', {x:150, y:12});
    TweenMax.set('.cloud2', {x:240, y:50});
    ...
    $('.frame-3 p').html('A CLOSER LOOK AT<br>LIVING IN LUXURY');
    $('.frame-4 p').html('MIX BUSINESS AND<br>LEISURE WITH THE AFR');
...
function start() {
    ...
    function featureAnimation() {
        tlFeature.to('.feature', 5, {ease: Back.easeOut.config(1), scale:0.5, x:110, y:-80})
                 .to('.cloud2', 6, {ease: Power1.easeInOut, x:"+=40", y:"-=20"}, "-=5")
